from fastapi import APIRouter, Form, Response, Request, Cookie, File, UploadFile, HTTPException
...
def generate_access_token(user_id: int):
    ...
    expiration = datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(seconds=30)
...
def get_profile(request: Request, access_token: str = Cookie(None)):
    if not access_token:  # Check if the token is missing
        return RedirectResponse(url="/users/login", status_code=303)
